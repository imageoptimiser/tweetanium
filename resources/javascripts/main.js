...
$MQL("l:app.compiled", function() {
	...
	Titanium.UI.currentWindow.setHeight(height + 25);
	...
	'		<a target="ti:systembrowser" href="http://twitter.com/#{user.screen_name}" alt="Go to profile for #{user.name}"><img height="48px" width="48px" src="#{user.profile_image_url}"/></a>'+
	'	</div>'+
	...
	'	</div>'+
	'	<div class="icon" row_id="#{id}" style="height:48px;width:48px" >'+
	'		<a target="ti:systembrowser" href="http://twitter.com/#{sender.screen_name}" alt="Go to profile for #{user.name}"><img heigth="48px" width="48px" src="#{sender.profile_image_url}"/></a>'+
	'	</div>'+
	'	<div class="message message_#{id}">#{display_text}</div>'+
	'   <div class="action_menu action_menu_#{id}" style="display:none">' +
	'		<div class="action_menu_container">' +
	...
	var Tweetanium2 = {
		...
		Create: function()
		{
			...
			{
		        Tweetanium2.db = openDatabase("tweetanium", "2.0");
				...
				{			
					...
					Tweetanium2.db.transaction(function(tx) 
					{
						...
						for (var c=0; c < tweets.length; c++)
						{
							...
							if (tweets[c].text)
							{
								tweets[c].text = tweets[c].text.gsub('"',"'");
								tweets[c].text = tweets[c].text.gsub('>','&gt;');
								tweets[c].text = tweets[c].text.gsub('<','&lt;');
							}
							...
							(function() {
								...
								tx.executeSql(Tweetanium2.sql.main.getTweet, [id], 
									function(tx, result)
									{
										if(result.rows.length < 1)
										{
											tx.executeSql(Tweetanium2.sql.main.saveTweet,[tweet,id,username]);
										}
									}
								);
							})();
				...
				success: function(replies,textStatus)
				{
					Tweetanium2.db.transaction(function(tx) 
					{
						...
						for (var c=0;c<replies.length;c++)
						{
							...
							if (replies[c].text)
							{
								replies[c].text = replies[c].text.gsub('"',"'");
								replies[c].text = replies[c].text.gsub('>','&gt;');
								replies[c].text = replies[c].text.gsub('<','&lt;');
							}
							...
							(function() {
								...
								tx.executeSql(Tweetanium2.sql.main.getReply, [id],
									function(tx, result)
									{
										if(result.rows.length < 1)
										{
											tx.executeSql(Tweetanium2.sql.main.saveReply, [tweet, id,username]);
										}
									}
								);
							})();
					...
					Tweetanium2.db.transaction(function(tx) 
					{
						...
						for (var c=0;c<dms.length;c++)
						{
							...
							(function() {
								...
								tx.executeSql(Tweetanium2.sql.main.getMessage,[id],
									function(tx, result)
									{
										if(result.rows.length < 1)
										{
											tx.executeSql(Tweetanium2.sql.main.saveMessage, [tweet, id,username]);
										}
									}
								);
							})();
		   ...
		   Tweetanium2.db.transaction(function(tx) 
			{
				tx.executeSql(
					...
					function(tx, result) 
					{
						...
						{
							jQuery('.all_paging_text').html('Showing '+ tweetArray.length + ' tweets');						
						}
		    ...
		    Tweetanium2.db.transaction(function(tx) 
			{
				tx.executeSql(
					...
					function(tx, result) 
					{
						...
						{
							jQuery('.replies_paging_text').html('Showing '+ tweetArray.length + ' replies');						
						}
		    ...
		    Tweetanium2.db.transaction(function(tx) 
			{
				tx.executeSql(
					...
					function(tx, result) 
					{
						...
						{
							jQuery('.dm_paging_text').html('Showing ' + tweetArray.length + ' direct messages');						
						}
	...
	jQuery('#go').click(function()
	{
		...
		else if (mode == 'DM')
		{
			...
			var tweetStr = tweet.split(' ');
			tweetStr = tweet.substring((tweetStr[0].length + tweetStr[1].length+1))
			jQuery.ajax(
			{
				'username':username,
				'password':password,
				...
				'data':{'text':tweetStr, 'user':user, 'source': 'tweetanium'},
				success:function(resp,textStatus)
				{
					jQuery('#tweettext').val('');
					displayLength();
